...
import { KafkaController } from '../impl/components/kafka/kafka.controller';
...
describe('KafkaController', () => {
  ...
  let kafkaServiceLog: KafkaServiceLog;
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        ...
        },
        {
          ...
          useValue: {
            sendMessageLog: jest.fn().mockResolvedValue(undefined),
          },
        },
        {
          provide: MessageTRX,
          useValue: {
            transform: jest.fn().mockReturnValue({ transformed: 'trxMessage' }),
          },
        },
        {
          provide: MessageTrxLog,
          useValue: {
            transform: jest.fn().mockReturnValue([{ transformed: 'logMessage' }]),
          },
        },
      ],
    }).compile();
    ...
    kafkaServiceLog = module.get<KafkaServiceLog>(KafkaServiceLog);
    messageTRX = module.get<MessageTRX>(MessageTRX);
    messageTrxLog = module.get<MessageTrxLog>(MessageTrxLog);
  });
  ...
  it('should throw BadRequestException if data is invalid', async () => {
    ...
    await expect(kafkaController.sendMessage(invalidMessageDto as any)).rejects.toThrow(BadRequestException);
  });

  it('should send message to Kafka successfully', async () => {
    const kafkaMessageDto: KafkaMessageDto = {
      level: 30,
      time: 1720801179486,
      name: 'paymentModel',
      pago: {
    ...
    await kafkaController.sendMessage(kafkaMessageDto);

    expect(messageTRX.transform).toHaveBeenCalledWith(kafkaMessageDto.pago);
    expect(kafkaService.sendMessage).toHaveBeenCalledWith({ transformed: 'trxMessage' });
    expect(messageTrxLog.transform).toHaveBeenCalledWith(kafkaMessageDto.pago.log);
    expect(kafkaServiceLog.sendMessageLog).toHaveBeenCalledWith({ transformed: 'logMessage' });
  });

  it('should throw BadRequestException if log_array is invalid', async () => {
    const invalidLogDto: KafkaMessageDto = {
      level: 30,
      time: 1720801179486,
      name: 'paymentModel',
      pago: {
    ...
    await expect(kafkaController.sendMessage(invalidLogDto)).rejects.toThrow(BadRequestException);
  });

  it('should send multiple log messages to Kafka successfully', async () => {
    const kafkaMessageDto: KafkaMessageDto = {
      level: 30,
      time: 1720801179486,
      name: 'paymentModel',
      pago: {
        ...
        log: [
          ...
          {
            idPago: '1603b7f5-debe-4452-af71-6abb9938db74',
            rrn: '765510425533',
            estado: 'RECHAZADO',
            infoAdicionalPOS: {
              canal: 'RED_MOVIL',
    ...
    expect(kafkaService.sendMessage).toHaveBeenCalledWith({ transformed: 'trxMessage' });
    expect(messageTrxLog.transform).toHaveBeenCalledWith(kafkaMessageDto.pago.log);
    expect(kafkaServiceLog.sendMessageLog).toHaveBeenCalledTimes(1);
    expect(kafkaServiceLog.sendMessageLog).toHaveBeenCalledWith({ transformed: 'logMessage' });
  });
});
